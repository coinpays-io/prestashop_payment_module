...
class CoinpaysTransaction
{
    ...
    {
        return DB::getInstance()->insert('coinpays_transaction', array(
    ...
    public static function getTransactionByoTokenByDupCartId($order_token, $id_dup_cart)
    {
        ...
        return Db::getInstance()->getRow("SELECT * FROM `" . _DB_PREFIX_ . "coinpays_transaction` WHERE `order_token` = '{$order_token}' AND `id_dup_cart` = '{$id_dup_cart}'");
    }
    ...
    public static function getTransactionByIdOidCartByOid($id_cart, $oid)
    {
        ...
        return Db::getInstance()->getRow("SELECT * FROM `" . _DB_PREFIX_ . "coinpays_transaction` WHERE `id_oid_cart` = {$id_cart} AND `merchant_oid` = '{$merchant_oid}'");
    }
    ...
    {
        $id_cart = (int)$id_cart;
        return Db::getInstance()->executeS("SELECT * FROM `" . _DB_PREFIX_ . "coinpays_transaction` WHERE `id_oid_cart` = {$id_cart}");
    }
    ...
    {
        $id_cart = (int)$id_cart;
        return Db::getInstance()->getRow("SELECT * FROM `" . _DB_PREFIX_ . "coinpays_transaction` 
    ...
    public static function updateTransactionIsOrderByCartIdByoToken($id_cart, $order_token)
    {
        ...
        Db::getInstance()->execute("UPDATE `" . _DB_PREFIX_ . "coinpays_transaction` SET `is_order` = 1, `date_updated` = '{$date}' WHERE `id_oid_cart` = {$id_cart} AND `order_token` = '{$order_token}'");
    }
    ...
    public static function updateTransactionIsComplete($id_cart, $merchant_oid, $status, $status_message, $total_paid, $is_notify_order = 0)
    {
        ...
        $date = date('Y-m-d H:i:s');
        Db::getInstance()->execute("UPDATE `" . _DB_PREFIX_ . "coinpays_transaction` SET 
        `is_complete` = 1, 
        `is_order` = {$is_order}, 
    ...
    public static function updateTransactionIsRefund($merchant_oid, $amount, $refund_status)
    {
        ...
        $date = date('Y-m-d H:i:s');
        return Db::getInstance()->execute("UPDATE `" . _DB_PREFIX_ . "coinpays_transaction` SET 
        `is_refunded` = 1, 
    ...
    # Update Invoice of the Order by order_id
    public static function updateOrderInvoice($number, $id_order, $note)
    {
        $number = (int)$number;
        $id_order = (int)$id_order;
        $note = pSQL($note);
        Db::getInstance()->execute("UPDATE `" . _DB_PREFIX_ . "order_invoice` 
        SET `note` = '{$note}' 
